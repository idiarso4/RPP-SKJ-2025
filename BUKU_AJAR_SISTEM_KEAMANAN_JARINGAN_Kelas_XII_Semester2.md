...
2.  **Langkah-langkah**:
    ...
    -   **Penutup (30 mnt)**:
        -   **Post-test Sederhana (15 mnt)**: Guru meminta siswa menuliskan 3 fase Cyber Kill Chain.
...
2.  **Langkah-langkah**:
    ...
    -   **Kegiatan Inti (135 mnt)**:
        ...
        -   **Praktik Terbimbing VM Pemula (90 mnt)**: Guru memandu siswa mengerjakan satu VM pemula dari VulnHub (misal: "Kiwi", "Basic Pentesting"). Siswa akan menerapkan teknik reconnaissance, vulnerability analysis, exploitation, dan privilege escalation untuk mendapatkan root/admin akses. (SEL: Manajemen diri, ketekunan).
    -   **Penutup (30 mnt)**:
...
#### **RPP - Pertemuan 6: Standar Kepatuhan Keamanan Informasi**
...
2.  **Langkah-langkah**:
    ...
    -   **Penutup (30 mnt)**:
        ...
        -   **Refleksi & Diferensiasi (15 mnt)**: Siswa menuliskan satu pelajaran penting dari simulasi insiden. Guru menawarkan sumber belajar tambahan tentang NIST SP 800-61 (Computer Security Incident Handling Guide). (SEL: Refleksi diri, manajemen diri).
...
#### **RPP - Pertemuan 11: Analisis Temuan & Penyusunan Laporan Proyek Akhir**
...
### **MODUL 4: PERSIAPAN KARIR & UJIAN KEJURUAN**
...
2.  **Langkah-langkah**:
    ...
    -   **Penutup (30 mnt)**:
        ...
        -   **Refleksi & Diferensiasi (15 mnt)**: Siswa menuliskan satu proyek yang ingin mereka tambahkan ke portofolio. Guru menawarkan umpan balik individual untuk portofolio siswa. (SEL: Refleksi diri, manajemen diri).
...
### LKPD Pertemuan 7: Incident Response
...
*   **Tahapan Utama Manajemen Risiko**:
    ...
    *   **5. Mitigasi Risiko (Penanganan Risiko)**: Mengambil tindakan untuk mengurangi risiko ke tingkat yang dapat diterima. Strategi mitigasi meliputi:
        *   **Accept (Menerima)**: Menerima risiko karena dampaknya rendah atau biaya mitigasi terlalu tinggi.
...
*   **Tahapan Incident Response (NIST SP 800-61)**:
    *   **1. Preparation (Persiapan)**:
        ...
        *   Contoh: Membuat Incident Response Plan, melatih tim, memiliki backup data.
    *   **2. Identification (Identifikasi)**:
        *   Deskripsi: Menentukan apakah suatu insiden telah terjadi, menganalisis indikator kompromi (IoC), dan menentukan ruang lingkup insiden.
        *   Contoh: Menganalisis log SIEM, alert IDS/IPS, laporan pengguna tentang anomali.
    *   **3. Containment (Pembatasan)**:
        *   Deskripsi: Mengisolasi sistem yang terpengaruh untuk mencegah penyebaran insiden lebih lanjut dan meminimalkan kerusakan.
        *   Contoh: Memutus koneksi jaringan, mematikan sistem yang terinfeksi, mengisolasi VM.
    *   **4. Eradication (Pemberantasan)**:
        *   Deskripsi: Menghilangkan akar penyebab insiden dan membersihkan sistem dari *malware*, *backdoor*, atau akses penyerang.
        *   Contoh: Menghapus *malware*, menambal kerentanan, membangun ulang sistem yang terkompromi.
    *   **5. Recovery (Pemulihan)**:
        *   Deskripsi: Mengembalikan sistem dan layanan ke operasi normal. Ini melibatkan pengujian, validasi, dan pemantauan untuk memastikan tidak ada sisa-sisa serangan.
        *   Contoh: Mengembalikan data dari backup, mengaktifkan kembali layanan, memantau kinerja.
    *   **6. Post-Incident Analysis (Analisis Pasca-Insiden)**:
        *   Deskripsi: Melakukan tinjauan menyeluruh terhadap insiden untuk mengidentifikasi pelajaran yang dapat diambil, meningkatkan proses IR, dan mencegah insiden serupa di masa depan.
        *   Contoh: Membuat laporan *lessons learned*, memperbarui kebijakan keamanan, meningkatkan kontrol keamanan.

#### Pertemuan 8: Disaster Recovery Plan (DRP) & Business Continuity Plan (BCP)

**1. Disaster Recovery Plan (DRP)**

DRP adalah dokumen terperinci yang berisi prosedur untuk memulihkan sistem dan infrastruktur IT setelah terjadi bencana (misal: kebakaran, banjir, serangan siber besar). Fokus utamanya adalah pada pemulihan teknologi.

*   **Metrik Kunci DRP**:
    *   **RTO (Recovery Time Objective)**: Waktu maksimum yang dapat diterima bagi sistem atau layanan untuk tidak tersedia setelah insiden. Ini adalah target waktu pemulihan.
    *   **RPO (Recovery Point Objective)**: Jumlah data maksimum yang dapat hilang setelah insiden. Ini menentukan seberapa sering backup harus dilakukan.

**2. Business Continuity Plan (BCP)**

BCP adalah rencana yang lebih luas yang memastikan bahwa fungsi bisnis penting dapat terus beroperasi selama dan setelah bencana. Ini mencakup aspek IT, operasional, SDM, keuangan, dan komunikasi.

*   **Perbedaan DRP dan BCP**: DRP adalah bagian dari BCP. DRP fokus pada pemulihan IT, sementara BCP fokus pada kelangsungan seluruh operasi bisnis.

**3. Praktik Backup & Restore Konfigurasi**

Backup dan restore adalah komponen krusial dari DRP. Memastikan konfigurasi perangkat jaringan dan server dapat dipulihkan dengan cepat sangat penting.

*   **Backup Konfigurasi Mikrotik**:
    *   CLI: `/system backup save name=myrouter_backup` (menyimpan file .backup)
    *   CLI: `/export file=myrouter_config` (menyimpan konfigurasi dalam format teks .rsc)
*   **Restore Konfigurasi Mikrotik**:
    *   CLI: `/system backup load name=myrouter_backup` (membutuhkan reboot)
    *   CLI: `/import file=myrouter_config.rsc` (tidak membutuhkan reboot, lebih fleksibel)
*   **Backup Konfigurasi VM Linux (Snapshot)**:
    *   Menggunakan fitur snapshot di VirtualBox/VMware untuk menyimpan keadaan VM pada titik waktu tertentu. Ini memungkinkan pemulihan cepat ke kondisi sebelumnya.
*   **Penyimpanan Backup**: Simpan backup di lokasi yang aman, terpisah dari sistem produksi (misal: penyimpanan eksternal, cloud storage).

### MODUL 3: PROYEK AKHIR (CAPSTONE)

#### Pertemuan 9: Penjelasan dan Perencanaan Proyek Akhir

**1. Proyek Akhir (Capstone)**

Proyek Capstone adalah puncak dari seluruh pembelajaran Sistem Keamanan Jaringan di kelas XI dan XII. Ini adalah proyek komprehensif yang mengharuskan peserta didik untuk mengintegrasikan semua pengetahuan dan keterampilan yang telah diperoleh untuk menyelesaikan masalah keamanan yang kompleks dalam skenario yang realistis.

*   **Tujuan Utama**: Menunjukkan kemampuan dalam melakukan *security assessment* (pengujian penetrasi dan audit keamanan) pada topologi simulasi yang mencakup berbagai komponen (jaringan, server, aplikasi web, container, IoT).

**2. Spesifikasi Proyek (Contoh)**

*   **Topologi Simulasi Kompleks**: Akan disediakan oleh guru atau dirancang oleh kelompok. Contoh: Jaringan dengan Mikrotik router, beberapa VM Linux (server web, server database, server autentikasi), container Docker, dan simulasi perangkat IoT.
*   **Lingkup *Assessment***: Melakukan *security assessment* menyeluruh pada topologi tersebut, mencakup:
    *   **Keamanan Jaringan**: Firewall rules, segmentasi, VPN.
    *   **Keamanan Sistem Operasi**: Hardening OS, manajemen patch, hak akses.
    *   **Keamanan Aplikasi Web**: Kerentanan OWASP Top 10 (SQL Injection, XSS, dll.).
    *   **Keamanan Container**: Konfigurasi Dockerfile, kerentanan image.
    *   **Keamanan IoT**: Kerentanan perangkat IoT, komunikasi.
*   **Output Proyek**: Laporan *security assessment* yang profesional dan presentasi/demonstrasi temuan.

**3. Perencanaan Proyek Kelompok**

*   **Pembagian Tugas**: Menentukan peran (misal: Lead Pentester, Vulnerability Analyst, Report Writer, Demo Lead) dan tanggung jawab masing-masing anggota.
*   **Timeline**: Membuat jadwal detail untuk setiap fase *assessment* (reconnaissance, vulnerability analysis, exploitation, reporting, presentation).
*   **Desain Topologi Target**: Jika merancang sendiri, buat diagram topologi yang jelas dengan detail IP, layanan, dan hubungan antar komponen.
*   **Strategi *Security Assessment***: Merencanakan pendekatan berdasarkan PTES dan Cyber Kill Chain. Memilih tools yang tepat untuk setiap fase.

#### Pertemuan 10: Pengerjaan Proyek Akhir (Implementasi Assessment)

**1. Implementasi *Security Assessment***

Pada tahap ini, kelompok akan secara aktif melakukan pengujian penetrasi dan audit keamanan pada topologi simulasi. Ini adalah fase *hands-on* yang paling intensif.

*   **Fase-fase yang Dilakukan**:
    *   **Reconnaissance**: Menggunakan Nmap, Wireshark, OSINT tools untuk memetakan jaringan, mengidentifikasi host aktif, port terbuka, layanan, dan informasi publik lainnya.
    *   **Vulnerability Analysis**: Menggunakan scanner kerentanan (Nessus/OpenVAS) dan melakukan analisis manual untuk mengidentifikasi kerentanan pada sistem operasi, aplikasi, dan konfigurasi.
    *   **Exploitation (Ethical)**: Memanfaatkan kerentanan yang ditemukan untuk mendapatkan akses. Ini bisa melibatkan penggunaan Metasploit, Burp Suite, skrip Python kustom, atau teknik manual lainnya. **Penting: Hanya lakukan eksploitasi yang diizinkan dan dalam batasan etika.**
    *   **Post-Exploitation**: Jika akses berhasil didapatkan, kumpulkan bukti, identifikasi data sensitif, dan tunjukkan dampak potensial.

**2. Pencatatan Progres dan Temuan**

Penting untuk mendokumentasikan setiap langkah, setiap temuan (kerentanan, bukti eksploitasi), dan setiap kendala yang dihadapi. Ini akan menjadi dasar laporan akhir.

*   **Log Aktivitas**: Catat perintah yang dijalankan, hasil output, dan waktu.
*   **Screenshot**: Ambil screenshot sebagai bukti temuan dan keberhasilan eksploitasi.
*   **Catatan Kerentanan**: Deskripsikan kerentanan, tingkat keparahan, dan potensi dampak.

#### Pertemuan 11: Analisis Temuan & Penyusunan Laporan Proyek Akhir

**1. Analisis Temuan**

Setelah fase *assessment* selesai, langkah krusial adalah menganalisis semua data yang terkumpul. Ini melibatkan:

*   **Klasifikasi Kerentanan**: Mengelompokkan kerentanan berdasarkan jenis (misal: SQL Injection, XSS, miskonfigurasi firewall) dan tingkat keparahan (Critical, High, Medium, Low) berdasarkan standar industri (misal: CVSS).
*   **Identifikasi Akar Masalah**: Menentukan penyebab mendasar dari kerentanan (misal: kurangnya validasi input, password default, software usang).
*   **Perumusan Rekomendasi**: Memberikan rekomendasi perbaikan yang jelas, spesifik, dan dapat ditindaklanjuti untuk setiap kerentanan. Rekomendasi harus realistis dan sesuai dengan konteks organisasi.

**2. Penyusunan Laporan *Security Assessment***

Laporan adalah output paling penting dari proyek Capstone. Ini harus profesional, jelas, dan persuasif.

*   **Komponen Laporan Umum**:
    *   **Ringkasan Eksekutif**: Ikhtisar singkat tentang tujuan *assessment*, temuan kunci, dan rekomendasi utama untuk manajemen.
    *   **Metodologi**: Menjelaskan pendekatan yang digunakan (misal: PTES, black-box/white-box).
    *   **Ruang Lingkup**: Menjelaskan apa yang diuji dan apa yang tidak.
    *   **Temuan Detail**: Daftar setiap kerentanan yang ditemukan, termasuk:
        *   Deskripsi kerentanan.
        *   Tingkat keparahan.
        *   Dampak potensial.
        *   Langkah-langkah reproduksi (bukti).
        *   Rekomendasi perbaikan.
    *   **Kesimpulan**: Ringkasan postur keamanan dan langkah selanjutnya.
    *   **Lampiran**: Screenshot, log, atau data teknis tambahan.

#### Pertemuan 12: Presentasi Proyek Akhir (Capstone)

**1. Presentasi dan Demonstrasi Proyek**

Ini adalah kesempatan bagi setiap kelompok untuk mempresentasikan hasil *security assessment* mereka kepada audiens (guru, teman-teman, atau bahkan perwakilan industri jika memungkinkan). Fokus pada komunikasi yang efektif dan demonstrasi yang jelas.

*   **Aspek Presentasi**:
    *   **Ringkasan Eksekutif**: Mulai dengan gambaran umum temuan dan rekomendasi.
    *   **Metodologi**: Jelaskan bagaimana *assessment* dilakukan.
    *   **Temuan Kunci**: Soroti kerentanan paling kritis dan dampaknya.
    *   **Demonstrasi**: Lakukan demo singkat (jika memungkinkan dan aman) untuk menunjukkan bagaimana kerentanan dieksploitasi.
    *   **Rekomendasi**: Sajikan rekomendasi perbaikan yang jelas.
    *   **Sesi Tanya Jawab**: Siap menjawab pertanyaan teknis dan strategis.

**2. Sesi Tanya Jawab dan Umpan Balik**

Setelah presentasi, akan ada sesi tanya jawab untuk memperdalam pemahaman dan memberikan umpan balik konstruktif. Ini adalah kesempatan untuk menunjukkan pemahaman teknis dan kemampuan komunikasi.

### MODUL 4: PERSIAPAN KARIR & UJIAN KEJURUAN

#### Pertemuan 13: Tinjauan Jalur Karir & Sertifikasi Industri

**1. Jalur Karir di Bidang Keamanan Siber**

Industri keamanan siber menawarkan berbagai peluang karir yang menarik dan terus berkembang.

*   **Contoh Profesi**:
    *   **Security Analyst (Analis Keamanan)**: Memantau sistem keamanan, menganalisis log, mendeteksi insiden, dan merespons ancaman.
    *   **Penetration Tester (Pentester)**: Melakukan pengujian penetrasi untuk menemukan kerentanan dalam sistem dan aplikasi.
    *   **SOC Analyst (Security Operations Center Analyst)**: Bekerja di pusat operasi keamanan, memantau, menganalisis, dan merespons insiden keamanan secara *real-time*.
    *   **Security Engineer**: Merancang, mengimplementasikan, dan memelihara arsitektur dan solusi keamanan.
    *   **Forensics Investigator (Penyidik Forensik Digital)**: Mengumpulkan dan menganalisis bukti digital setelah insiden untuk menentukan apa yang terjadi.
    *   **Security Consultant**: Memberikan saran ahli kepada organisasi tentang strategi keamanan, kepatuhan, dan implementasi solusi.

**2. Sertifikasi Industri**

Sertifikasi profesional sangat dihargai di industri keamanan siber karena menunjukkan pengetahuan dan keterampilan yang teruji.

*   **Contoh Sertifikasi Populer**:
    *   **CompTIA Security+**: Sertifikasi tingkat dasar yang mencakup konsep keamanan inti, manajemen risiko, kriptografi, dan operasi keamanan. Baik untuk pemula.
    *   **CEH (Certified Ethical Hacker)**: Sertifikasi yang berfokus pada teknik dan metodologi *ethical hacking*.
    *   **OSCP (Offensive Security Certified Professional)**: Sertifikasi *hands-on* yang sangat dihormati untuk *penetration testing*, membutuhkan kemampuan eksploitasi nyata.
    *   **CISSP (Certified Information Systems Security Professional)**: Sertifikasi tingkat lanjut untuk profesional keamanan yang berpengalaman, mencakup berbagai domain keamanan.

#### Pertemuan 14: Penyusunan Portofolio Digital

**1. Pentingnya Portofolio Digital**

Dalam industri IT, terutama keamanan siber, portofolio digital sama pentingnya dengan ijazah. Ini adalah kumpulan bukti nyata dari keterampilan, proyek, dan pengalaman Anda yang dapat ditunjukkan kepada calon pemberi kerja.

*   **Mengapa Penting?**:
    *   **Bukti Keterampilan**: Menunjukkan apa yang benar-benar dapat Anda lakukan, bukan hanya apa yang Anda ketahui secara teori.
    *   **Personal Branding**: Membangun reputasi profesional Anda.
    *   **Diferensiasi**: Membedakan Anda dari kandidat lain.

**2. Platform untuk Portofolio Digital**

*   **GitHub**: Ideal untuk menampilkan proyek-proyek pemrograman, skrip keamanan, atau kontribusi *open-source*. Pastikan repositori Anda rapi, terdokumentasi, dan berisi README yang jelas.
*   **LinkedIn**: Platform jaringan profesional. Optimalkan profil Anda dengan mencantumkan keterampilan, pengalaman, sertifikasi, dan tautan ke proyek-proyek Anda.
*   **Blog Pribadi**: Tempat untuk menulis artikel teknis, *write-up* CTF, atau refleksi tentang pembelajaran Anda. Menunjukkan kemampuan komunikasi dan pemahaman mendalam.

**3. Konten Portofolio yang Direkomendasikan**:

*   **Proyek Praktis**: Hasil proyek dari sekolah (misal: proyek Capstone, skrip Python, konfigurasi lab).
*   **Write-up CTF/VulnHub**: Dokumentasi langkah-langkah Anda dalam menyelesaikan tantangan hacking.
*   **Sertifikasi**: Cantumkan sertifikasi yang telah Anda peroleh.
*   **Keterampilan Teknis**: Daftar tools, bahasa pemrograman, dan teknologi yang Anda kuasai.
*   **Pengalaman Relevan**: Magang, kerja paruh waktu, atau pengalaman sukarela di bidang keamanan.

#### Pertemuan 15: Persiapan Ujian Kompetensi Kejuruan (UKK)

**1. Ujian Kompetensi Kejuruan (UKK)**

UKK adalah ujian nasional bagi siswa SMK untuk mengukur pencapaian kompetensi sesuai dengan standar yang ditetapkan. Bagi program keahlian Sistem Informasi Jaringan dan Aplikasi, UKK akan menguji keterampilan praktis dan pengetahuan teoritis di bidang keamanan jaringan.

*   **Format UKK (Umum)**:
    *   **Ujian Teori**: Soal pilihan ganda atau esai yang menguji pemahaman konsep dasar, metodologi, dan standar keamanan.
    *   **Ujian Praktik**: Skenario *hands-on* yang mengharuskan siswa untuk mengkonfigurasi, mengamankan, atau menganalisis sistem jaringan menggunakan tools yang relevan.

*   **Materi Kunci UKK (Sistem Keamanan Jaringan)**:
    *   Konsep dasar keamanan (CIA Triad, Defense in Depth, Least Privilege).
    *   Ancaman dan kerentanan (malware, serangan jaringan, social engineering).
    *   Firewall (filter, NAT).
    *   Autentikasi terpusat (RADIUS, AAA).
    *   Deteksi intrusi dan analisis log (IDS/IPS, SIEM).
    *   Kriptografi dan VPN.
    *   Keamanan aplikasi web (OWASP Top 10, SQL Injection, XSS).
    *   Keamanan cloud dan container.
    *   Ethical hacking dan penetration testing (reconnaissance, scanning, exploitation).
    *   Manajemen risiko, incident response, DRP/BCP.

**2. Strategi Persiapan UKK**:

*   **Review Materi**: Pelajari kembali semua modul dari kelas XI dan XII.
*   **Latihan Praktik**: Perbanyak latihan *hands-on* dengan lab virtual, TryHackMe, VulnHub.
*   **Pahami Soal**: Pelajari contoh soal UKK tahun-tahun sebelumnya.
*   **Manajemen Waktu**: Latih diri untuk menyelesaikan tugas dalam batas waktu yang ditentukan.
*   **Troubleshooting**: Tingkatkan kemampuan *troubleshooting* karena ini krusial dalam ujian praktik.

#### Pertemuan 16: Evaluasi Akhir Semester & Refleksi Karir

**1. Evaluasi Akhir Semester**

Ini adalah kesempatan untuk merefleksikan perjalanan belajar selama semester terakhir dan seluruh kelas XII. Evaluasi ini membantu siswa mengidentifikasi kekuatan, area yang perlu ditingkatkan, dan bagaimana pembelajaran telah membentuk mereka.

*   **Aspek Evaluasi Diri**:
    *   Konsep yang paling dikuasai dan yang masih sulit.
    *   Keterampilan praktik yang meningkat dan yang perlu diasah.
    *   Metode pembelajaran yang paling efektif.
    *   Kontribusi dalam kerja kelompok.

**2. Refleksi Karir**

Setelah menyelesaikan seluruh materi keamanan siber, siswa didorong untuk merenungkan tujuan karir mereka dan langkah-langkah selanjutnya.

*   **Pertanyaan Refleksi**:
    *   Apa yang paling menarik dari bidang keamanan siber bagi Anda?
    *   Profesi apa yang paling sesuai dengan minat dan keterampilan Anda?
    *   Apa tujuan jangka pendek (misal: mendapatkan sertifikasi, magang) dan jangka panjang (misal: menjadi ahli di bidang tertentu) Anda?
    *   Bagaimana Anda akan terus belajar dan mengembangkan diri setelah lulus?

*   **Pentingnya Pembelajaran Berkelanjutan**: Bidang keamanan siber terus berkembang. Penting untuk memiliki mentalitas pembelajar seumur hidup untuk tetap relevan.

## LAMPIRAN - GLOSARIUM

*   **Actions on Objectives (Cyber Kill Chain)**: Fase terakhir Cyber Kill Chain di mana penyerang mencapai tujuan akhir mereka (misal: pencurian data, perusakan sistem).
*   **Business Continuity Plan (BCP)**: Rencana yang lebih luas yang memastikan fungsi bisnis penting dapat terus beroperasi selama dan setelah bencana.
*   **Cyber Kill Chain**: Model yang menggambarkan tahapan serangan siber dari perspektif penyerang (Reconnaissance, Weaponization, Delivery, Exploitation, Installation, Command & Control, Actions on Objectives).
*   **Command & Control (C2) (Cyber Kill Chain)**: Fase Cyber Kill Chain di mana penyerang membangun saluran komunikasi rahasia dengan sistem yang dikompromikan untuk mengendalikan *malware* dari jarak jauh.
*   **Containment (Incident Response)**: Tahapan Incident Response untuk mengisolasi sistem yang terpengaruh untuk mencegah penyebaran insiden lebih lanjut.
*   **Disaster Recovery Plan (DRP)**: Dokumen terperinci yang berisi prosedur untuk memulihkan sistem dan infrastruktur IT setelah terjadi bencana.
*   **Delivery (Cyber Kill Chain)**: Fase Cyber Kill Chain di mana senjata siber dikirimkan ke target (misal: email phishing, drive-by download).
*   **Eradication (Incident Response)**: Tahapan Incident Response untuk menghilangkan akar penyebab insiden dan membersihkan sistem dari *malware* atau akses penyerang.
*   **Ethical Hacking**: Simulasi serangan siber yang sah dan terotorisasi untuk menemukan kerentanan keamanan.
*   **Exploitation (Cyber Kill Chain)**: Fase Cyber Kill Chain di mana senjata siber dieksekusi pada sistem target, memanfaatkan kerentanan untuk mendapatkan akses.
*   **Exploitation (PTES)**: Fase PTES di mana *penetration tester* memanfaatkan kerentanan yang ditemukan untuk mendapatkan akses ke sistem target.
*   **Identification (Incident Response)**: Tahapan Incident Response untuk menentukan apakah suatu insiden telah terjadi dan menganalisis ruang lingkupnya.
*   **Incident Response (IR)**: Proses terstruktur yang diikuti oleh organisasi untuk menangani dan mengelola konsekuensi dari insiden keamanan siber.
*   **Intelligence Gathering (PTES)**: Fase PTES untuk mengumpulkan informasi tentang target menggunakan berbagai teknik (OSINT, scanning).
*   **Installation (Cyber Kill Chain)**: Fase Cyber Kill Chain di mana penyerang menginstal *backdoor* atau *persistence mechanism* pada sistem target.
*   **ISO/IEC 27001**: Standar internasional untuk Sistem Manajemen Keamanan Informasi (SMKI).
*   **Manajemen Risiko Keamanan Informasi**: Proses sistematis untuk mengidentifikasi, menganalisis, mengevaluasi, dan mengelola risiko terhadap aset informasi organisasi.
*   **OSINT (Open Source Intelligence)**: Pengumpulan informasi dari sumber-sumber yang tersedia untuk umum.
*   **Penetration Testing Execution Standard (PTES)**: Standar komprehensif untuk melakukan pengujian penetrasi, membagi proses menjadi tujuh fase.
*   **Post-Exploitation (PTES)**: Fase PTES di mana *penetration tester* mempertahankan akses, mengumpulkan informasi lebih lanjut, dan melakukan *privilege escalation*.
*   **Post-Incident Analysis (Incident Response)**: Tahapan Incident Response untuk melakukan tinjauan menyeluruh terhadap insiden untuk mengidentifikasi pelajaran yang dapat diambil.
*   **Preparation (Incident Response)**: Tahapan Incident Response untuk membangun tim IR, mengembangkan kebijakan, dan memastikan sistem siap untuk deteksi dan respon.
*   **Pre-engagement Interactions (PTES)**: Fase awal PTES di mana *penetration tester* dan klien mendefinisikan ruang lingkup, tujuan, dan legalitas pengujian.
*   **Recovery (Incident Response)**: Tahapan Incident Response untuk mengembalikan sistem dan layanan ke operasi normal.
*   **Reconnaissance (Cyber Kill Chain)**: Fase awal Cyber Kill Chain di mana penyerang mengumpulkan informasi tentang target.
*   **Reporting (PTES)**: Fase PTES di mana *penetration tester* menyusun laporan komprehensif tentang temuan, dampak, dan rekomendasi perbaikan.
*   **RPO (Recovery Point Objective)**: Jumlah data maksimum yang dapat hilang setelah insiden, menentukan seberapa sering backup harus dilakukan.
*   **RTO (Recovery Time Objective)**: Waktu maksimum yang dapat diterima bagi sistem atau layanan untuk tidak tersedia setelah insiden.
*   **Security Assessment**: Proses evaluasi keamanan sistem atau organisasi untuk mengidentifikasi kerentanan dan risiko.
*   **Threat Modeling (PTES)**: Fase PTES untuk menganalisis informasi yang dikumpulkan untuk mengidentifikasi potensi ancaman dan kerentanan yang paling mungkin dieksploitasi.
*   **UKK (Ujian Kompetensi Kejuruan)**: Ujian nasional bagi siswa SMK untuk mengukur pencapaian kompetensi sesuai standar yang ditetapkan.
*   **Vulnerability Analysis (PTES)**: Fase PTES untuk mengidentifikasi dan memverifikasi kerentanan yang ada pada sistem target.
*   **VulnHub**: Platform yang menyediakan mesin virtual yang sengaja dibuat rentan untuk tujuan pembelajaran *ethical hacking*.
*   **Weaponization (Cyber Kill Chain)**: Fase Cyber Kill Chain di mana penyerang membuat "senjata" siber dengan menggabungkan *malware* dengan *exploit*.
...
*   Allsopp, William. (2009). *Advanced Penetration Testing: Hacking the World's Most Secure Networks*. Addison-Wesley Professional.
*   Conklin, Wm. Arthur, White, Greg, Cothren, Dwayne, Davis, Roger, & Williams, Chuck. (2015). *Principles of Computer Security: CompTIA Security+ and Beyond*. McGraw-Hill Education.

### Dokumentasi Resmi Vendor/Organisasi
*   **PTES (Penetration Testing Execution Standard)**: The Penetration Testing Execution Standard.
    *   [http://www.pentest-standard.org/](http://www.pentest-standard.org/)
*   **Cyber Kill Chain (Lockheed Martin)**: Lockheed Martin Cyber Kill Chain Framework.
...
*   **NIST SP 800-30 Rev. 1**: Guide for Conducting Risk Assessments. National Institute of Standards and Technology.
    *   [https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-30r1.pdf](https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-30r1.pdf)
...
*   **ISO/IEC 27005**: Information security risk management. International Organization for Standardization.
    *   [https://www.iso.org/isoiec-27005-information-security.html](https://www.iso.org/isoiec-27005-information-security.html)
*   **GDPR (General Data Protection Regulation)**: Official EU GDPR Portal.
...
*   **OWASP.org**: Berbagai proyek dan sumber daya keamanan aplikasi web.
    *   [https://owasp.org/](https://owasp.org/)
*   **YouTube Channels**: Berbagai kanal yang menyediakan tutorial dan penjelasan tentang ethical hacking, manajemen keamanan, dan persiapan karir (misal: The Cyber Mentor, John Hammond, David Bombal).
